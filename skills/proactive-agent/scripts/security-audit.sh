...
if [ -f "AGENTS.md" ]; then
    # Accept either explicit prompt-injection wording OR strong data-exfiltration safeguards
    if grep -qiE "injection|external content|never execute|exfiltrat|private data" "AGENTS.md"; then
        pass "AGENTS.md contains security guardrails (injection/exfiltration)"
    else
        warn "AGENTS.md may be missing prompt-injection or exfiltration defense"
    fi
...
# 6. Check .gitignore (only meaningful in git repos)
...
if git rev-parse --is-inside-work-tree >/dev/null 2>&1; then
    if [ -f ".gitignore" ]; then
        ...
        warn "No .gitignore found in git repo"
    fi
else
    pass "Not a git repository; skipping .gitignore checks"
fi
echo ""
